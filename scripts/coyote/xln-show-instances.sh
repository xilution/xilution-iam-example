...
curl -s \
  ...
  "https://${environment}.coyote.content-delivery.api.xilution.com/organizations/${sub_organization_id}/instances?page-number=0&page-size=25" |
  jq '.content | if (. | length > 0) then (.[] | {id: .id, name: .name}) else "No instances found." end'
